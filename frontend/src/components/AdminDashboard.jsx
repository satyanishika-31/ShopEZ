...
const AdminDashboard = () => {
  ...
  const [newProduct, setNewProduct] = useState({ symbol: '', name: '', price: '', imageUrl: '', category: 'Smart Living' });
  ...
  const handleCreateProduct = async (e) => {
    ...
    try {
      ...
      setNewProduct({ symbol: '', name: '', price: '', imageUrl: '', category: 'Smart Living' });
      fetchSellerData();
    } catch (err) {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      await API.put(`/admin/stocks/${editingProduct.symbol}`, {
        ...
        category: editingProduct.category || 'Smart Living'
      });
          ...
          <>
            ...
            <div className="glass-card flex flex-col gap-5 rounded-2xl border border-darkBorder p-6 lg:col-span-1">
              ...
              <form onSubmit={editingProduct ? handleUpdateProduct : handleCreateProduct} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold uppercase tracking-wider text-textSecondary">Category</label>
                  <select
                    value={editingProduct ? editingProduct.category || 'Smart Living' : newProduct.category}
                    onChange={(e) =>
                      editingProduct
                ...
                <table className="table-custom w-full border-collapse text-left">
                  <thead>
                    <tr className="bg-darkBg/20">
                      ...
                      <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-textMuted">Category</th>
                      {isAdmin && (
                  ...
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={isAdmin ? 8 : 7} className="py-12 text-center text-xs text-textMuted">Fetching listings...</td>
                      </tr>
                    ) : products.length > 0 ? (
                      products.map((product) => (
                        <tr key={product.symbol} className="border-b border-darkBorder/40">
                          ...
                          <td className="px-6 py-3.5 text-sm text-textSecondary font-semibold">
                      ...
                      <tr>
                        <td colSpan={isAdmin ? 8 : 7} className="py-12 text-center text-sm text-textSecondary">No products found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
